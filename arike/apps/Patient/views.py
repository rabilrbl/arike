    ...
    def get_queryset(self):
        if self.request.user.role == 1:
            queryset = Patient.objects.filter(
                ward__local_body__district=self.request.user.district
            )
        else:
...
class PatientDetailView(PermissionRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["last_visit"] = (
            VisitSchedule.objects.filter(patient=self.object).order_by("-date").first()
        )
...
class PatientForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["gender"] = forms.ChoiceField(
            widget=widgets.AdminRadioSelect, choices=choices.GENDER
        )
